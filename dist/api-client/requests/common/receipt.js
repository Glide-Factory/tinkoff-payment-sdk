...
function validateAndPrepareReceipts(receiptsArray) {
    const finalReceipts = [];
    for (const receipt of receiptsArray) {
        if (!receipt.Items) {
            throw new Error('Receipt.Items must be set when receipt is used');
        }
        if (receipt.Items.length === 0) {
        ...
        finalReceipts.push(Object.assign(Object.assign({}, receipt), { Items: [...receipt.Items.map(validateAndPrepareReceiptItem)] }));
    }
    return finalReceipts;
}
exports.validateAndPrepareReceipts = validateAndPrepareReceipts;
